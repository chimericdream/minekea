...
public class GlassJarBlockEntityRenderer implements BlockEntityRenderer<GlassJarBlockEntity> {
    ...
    private void renderFluid(GlassJarBlockEntity entity, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        if (storedFluid.matchesType(Fluids.EMPTY) || world == null) {
            return;
        }

        IClientFluidTypeExtensions renderProperties = IClientFluidTypeExtensions.of(storedFluid);
        int color = renderProperties.getTintColor(storedFluid.getDefaultState(), world, entity.getPos());
